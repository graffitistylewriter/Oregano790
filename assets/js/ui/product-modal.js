...
const OreganoProductModal = (() => {
    ...
    const injectStyles = () => {
        ...
        style.textContent = `
            ...
            .oregano-product-modal {
                ...
                padding: 24px; background: rgba(0,0,0,.78); backdrop-filter: blur(10px);
            }
            .oregano-product-modal__dialog {
                position: relative; width: min(980px, 100%); max-height: min(820px, calc(100vh - 48px));
                overflow: auto; display: grid; grid-template-columns: minmax(280px, .9fr) minmax(320px, 1.1fr);
                background: #171717; color: #fff; border: 1px solid rgba(255,255,255,.10);
                border-radius: 28px; box-shadow: 0 40px 120px rgba(0,0,0,.55);
            }
            ...
            @media (max-width: 760px) {
                ...
                .oregano-product-modal__image, .oregano-product-modal__image img { min-height: 260px; height: 260px; }
    ...
    const findProduct = async (card) => {
        ...
        return fallback?.getProductById(productId)
            || fallback?.getProducts().find(product => String(product.name).trim().toLowerCase() === name.toLowerCase())
            || null;
    };
    ...
    const renderProduct = product => {
        ...
        const alt = typeof product.image === "object" && typeof product.image?.alt === "string"
            ? product.image.alt
            : product.name;
