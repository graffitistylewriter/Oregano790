...
/**
 * Product-detail modal for the canonical catalogue.
 *
 * The modal deliberately resolves products through the existing
 * OreganoCatalogueService boundary. It does not read or create a
...
const OreganoProductModal = (() => {
    ...
    const injectStyles = () => {
        ...
        style.textContent = `
            ...
            .oregano-product-modal__image {
                min-height: 420px;
                background: #222;
            }
            .oregano-product-modal__image img {
                width: 100%;
                height: 100%;
                min-height: 420px;
                display: block;
            ...
            .oregano-product-modal__content {
                padding: clamp(28px, 5vw, 56px);
                display: flex;
                flex-direction: column;
                justify-content: center;
            }
            .oregano-product-modal__eyebrow {
                margin-bottom: 12px;
                color: #aeb69f;
                font-size: .72rem;
                font-weight: 700;
            ...
            .oregano-product-modal__title {
                margin: 0 52px 18px 0;
                font-family: "Cormorant Garamond", serif;
                font-size: clamp(2.6rem, 5vw, 4.5rem);
                line-height: .95;
            }
            .oregano-product-modal__meta {
                display: flex;
                flex-wrap: wrap;
                gap: 8px;
                margin-bottom: 24px;
            }
            .oregano-product-modal__meta span {
                padding: 7px 11px;
                border: 1px solid rgba(255,255,255,.10);
                border-radius: 999px;
                font-size: .72rem;
            ...
            .oregano-product-modal__description {
                margin-bottom: 28px;
                color: rgba(255,255,255,.76);
                line-height: 1.85;
            }
            .oregano-product-modal__details {
                display: grid;
                grid-template-columns: repeat(2, minmax(0,1fr));
                gap: 12px;
                margin-bottom: 30px;
            }
            .oregano-product-modal__detail {
                padding: 14px 16px;
                border-radius: 14px;
                background: rgba(255,255,255,.045);
                border: 1px solid rgba(255,255,255,.06);
            }
            .oregano-product-modal__detail small {
                display: block;
                margin-bottom: 5px;
                opacity: .55;
                font-size: .66rem;
            ...
            .oregano-product-modal__price {
                margin-bottom: 20px;
                color: #aeb69f;
                font-size: 2rem;
                font-weight: 700;
            }
            .oregano-product-modal__close {
                position: absolute;
                top: 18px;
                right: 18px;
                z-index: 2;
            ...
            .oregano-product-modal__close:hover,
            .oregano-product-modal__close:focus-visible { background: #59624c; }
            body.oregano-modal-open { overflow: hidden; }
            @media (max-width: 760px) {
                .oregano-product-modal { padding: 12px; }
                .oregano-product-modal__dialog { grid-template-columns: 1fr; max-height: calc(100vh - 24px); }
                .oregano-product-modal__image,
                .oregano-product-modal__image img { min-height: 260px; height: 260px; }
                .oregano-product-modal__content { padding: 28px 22px 32px; }
                .oregano-product-modal__details { grid-template-columns: 1fr 1fr; }
            }
        `;
    ...
    const ensureModal = () => {
        ...
        modal.addEventListener("click", event => {
            if (event.target === modal || event.target.closest(".oregano-product-modal__close")) close();
        });
    ...
    const findProduct = async (card) => {
        ...
        return fallback?.getProducts().find(product => String(product.name).trim().toLowerCase() === name.toLowerCase()) || null;
    };
    ...
    const renderProduct = product => {
        ...
        const imageSource = typeof product.image === "string" ? product.image : product.image?.src;
        const alt = typeof product.image === "object" ? product.image?.alt : product.name;

        image.innerHTML = `<img src="${escapeHtml(imageSource || "assets/images/product-placeholder.svg")}" alt="${escapeHtml(alt || product.name)}">`;
        content.innerHTML = `
            ...
            <div class="oregano-product-modal__meta">
